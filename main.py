...
# -------------------- AUTH GATE --------------------
st.set_page_config(page_title="GitHub GPT-4o Agent", page_icon="💡")

if "github_token" not in st.session_state:
    st.title("🔐 GitHub GPT-4o Agent")
    st.markdown("""
    This app uses a GPT-4o model served by GitHub.

    Please enter your **GitHub token** to continue.

    - You can get a token from: [https://github.com/settings/tokens](https://github.com/settings/tokens)
    """)

    token_input = st.text_input("Enter your GitHub Token:", type="password")

    if st.button("Start Chatting") and token_input:
        # Try to invoke the model using token
        try:
            temp_agent = get_agent(token_input)
            temp_agent.invoke({"messages": [HumanMessage(content="Hello")]})
...
with st.sidebar:
    ...
    if st.button("🔒 Log out"):
        for k in ["github_token", "user_hash", "session_id", "messages"]:
...
if user_input:
    ...
    agent = get_agent(st.session_state["github_token"])
